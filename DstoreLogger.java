...
public class DstoreLogger extends Logger {
   private static DstoreLogger loggerInstance = null;
   private int port;

   public static synchronized void init(LoggingType loggingType, int port) throws IOException {
      if (loggerInstance == null) {
         loggerInstance = new DstoreLogger(loggingType);
         loggerInstance.port = port;
      } else {
         throw new RuntimeException("DstoreLogger already initialised");
      }
   ...
   protected DstoreLogger(LoggingType loggingType) {
      super(loggingType);
   ...
   public void errorConnecting(int cport) {
      this.log("Cannot connect to the ControllerTest on port ".concat(String.valueOf(cport)));
   }
   ...
   public void dstoresWhereToStoreTo(String filename, int[] dstorePorts) {
      StringBuffer filename1 = new StringBuffer("ControllerTest replied to store " + filename + " in these Dstores: ");
   ...
   public void storeToDstoreStarted(String filename, int dstorePort) {
      this.log("Storing file " + filename + " to DstoreTest " + dstorePort);
   }

   public void ackFromDstore(String filename, int dstorePort) {
      this.log("ACK received from DstoreTest " + dstorePort + " to store file " + filename);
   }

   public void storeToDstoreCompleted(String filename, int dstorePort) {
      this.log("Store of file " + filename + " to DstoreTest " + dstorePort + " successfully completed");
   }

   public void storeToDstoreFailed(String filename, int dstorePort) {
      this.log("Store of file " + filename + " to DstoreTest " + dstorePort + " failed");
   }
   ...
   public void dstoreWhereToLoadFrom(String filename, int dstorePort, int filesize) {
      this.log("ControllerTest replied to load file " + filename + " (size: " + filesize + " bytes) from DstoreTest " + dstorePort);
   }

   public void loadFromDstore(String filename, int dstorePort) {
      this.log("Loading file " + filename + " from DstoreTest " + dstorePort);
   }

   public void loadFromDstoreFailed(String filename, int dstorePort) {
      this.log("Load operation for file " + filename + " from DstoreTest " + dstorePort + " failed");
   }
   ...
   public void loadCompleted(String filename, int dstoreCount) {
      this.log("Load operation of file " + filename + " from DstoreTest " + dstoreCount + " successfully completed");
   }
